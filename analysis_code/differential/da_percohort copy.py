...
Country groups tested:
  WEST  — FRA, ITA, JPN, USA, CAN  (pooled into one stratum)
  AUT   — Austria (own stratum)
  CHN   — China   (own stratum)
  DEU   — Germany (own stratum)
...
from da_shared import (
    ...
    standard_wilcoxon_p, blocked_wilcoxon_permutation_p, _short,
)
...
def _da_one_group(clr_sub, orig_sub, labels, df_meta_sub,
                  group_a="control", group_b="CRC"):
    """
    Run DA for a single country group.

    Automatically selects the statistical method based on how many
    datasets are present in the group:

    - 1 dataset  → plain Wilcoxon (exact distribution for small n)
                   + simple OLS (no dataset dummies needed)
    - >1 datasets → blocked permutation Wilcoxon (blocks = dataset)
                   + OLS with dataset dummies to partial out study effects
    """
    ...
    condition_arr = (labels == group_b).astype(float)
    blocks        = df_meta_sub["dataset_name"].values
    n_datasets    = df_meta_sub["dataset_name"].nunique()
    use_blocking  = n_datasets > 1

    if use_blocking:
        ds_dummies = pd.get_dummies(
            df_meta_sub["dataset_name"], drop_first=True
        ...
        ols_coef_key = "condition_CRC"
    else:
        X_ols        = sm.add_constant(
            pd.Series(condition_arr, name="condition_CRC")
        )
        ols_coef_key = "condition_CRC"
    ...
    for taxon in clr_sub.columns:
        ...
        try:
            ...
            coef   = float(model.params[ols_coef_key])
            se     = float(model.bse[ols_coef_key])
            t_stat = float(model.tvalues[ols_coef_key])
            p_ols  = float(model.pvalues[ols_coef_key])
        except Exception:
            coef = se = t_stat = p_ols = np.nan

        if use_blocking:
            p_val = blocked_wilcoxon_permutation_p(
                x, labels, blocks=blocks,
                group_a=group_a, group_b=group_b,
                n_perm=N_PERMUTATIONS, random_state=RANDOM_SEED,
            )
        else:
            p_val = standard_wilcoxon_p(x, labels, group_a, group_b)
        ...
        records.append({
            ...
            "p_val"      : p_val,  "wilcoxon_p": p_val,
            "auroc"      : auroc,  "gfc"       : gfc,
            "n_ctrl"     : int(mask_a.sum()),
            "n_crc"      : int(mask_b.sum()),
            "n_datasets" : n_datasets,
            "method"     : "blocked_permutation" if use_blocking else "wilcoxon_exact",
        })
...
def run_percountry_da(df_feat, df_meta):
    ...
    re-normalise, CLR-transform, and run DA.

    Method is chosen automatically per group:
      - >1 datasets in group -> blocked permutation Wilcoxon + OLS with dataset dummies
      - 1 dataset  in group -> standard Wilcoxon (exact) + simple OLS
    ...
    for gname, countries in COUNTRY_GROUPS.items():
        ...
        n_datasets = int(df_meta.loc[mask_cc, "dataset_name"].nunique())
        method_str = "blocked permutation" if n_datasets > 1 else "exact Wilcoxon"

        print(f"\n  [{gname}] {GROUP_LABEL_MAP[gname]}")
        print(f"    ctrl={n_ctrl}  CRC={n_crc}  "
              f"datasets={n_datasets}  -> using {method_str}")
        ...
        meta_cc    = df_meta.loc[valid_rows[valid_rows].index].copy().reset_index(drop=True)
        orig_cc    = orig_cc.reset_index(drop=True)
        labels_cc  = labels_cc.reset_index(drop=True)
        ...
        res    = _da_one_group(clr_cc, orig_cc, labels_cc, meta_cc)
