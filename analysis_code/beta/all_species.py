...
if unmapped.sum() > 0:
    ...
    print(
        f"  Excluding {unmapped.sum()} sample(s) with "
...
# Re-derive metadata and species-only views from the train set.
# species_cols excludes every known metadata column (crc_label was already
# dropped by split_dataset).
...
species_df = train_df[species_cols].apply(pd.to_numeric, errors="coerce").fillna(0)
...
species_rel = species_norm[species_cols]

# Post-normalisation sanity check: every row should now sum to 1.0
...
if bad_prop.sum() > 0:
    ...
    keep = ~bad_prop
    metadata = metadata[keep.values].reset_index(drop=True)
    species_rel = species_rel[keep.values].reset_index(drop=True)
else:
    print(
...
# 7a. PERMANOVA on dataset_name
print("\n  7a. PERMANOVA on dataset_name (batch effect)...")

perm_batch_cc = permanova(
    dm_cc, meta_cc, column="dataset_name", permutations=PERMUTATIONS
)
...
try:
    ...
    print(
        f"    CC batch PERMDISP  — p={fmt(p_disp_batch_cc)}  {sig_stars(p_disp_batch_cc)}"
    )
except Exception as e:
    print(f"  CC batch PERMDISP failed: {e}")
    p_disp_batch_cc = np.nan

# 7b. Partial PERMANOVA — condition | dataset
...
perm_partial_cc = permanova(
    dm_resid_cc, meta_cc, column="study_condition", permutations=PERMUTATIONS
)
...
print(
    f"      Condition alone : {vp_cc['condition']:.4f}  ({vp_cc['condition']*100:.1f}%)"
)
print(f"      Shared          : {vp_cc['shared']:.4f}  ({vp_cc['shared']*100:.1f}%)")
print(
    f"      Residual        : {vp_cc['residual']:.4f}  ({vp_cc['residual']*100:.1f}%)"
)
print(
    f"      Total explained : {vp_cc['total_explained']:.4f}  ({vp_cc['total_explained']*100:.1f}%)"
)


# 8. PRINT FULL RESULTS SUMMARY
print("\n" + "=" * 75)
print("  FULL STATISTICAL RESULTS  |  Train set (80% per dataset)  |  Species level")
print(
    "  ω² interpretation: <0.01 negligible | 0.01–0.06 small | "
...
print(
    f"    Omega²       = {fmt(omega2_batch_cc, 4)}  ({_omega_label(omega2_batch_cc)})"
)
print(f"    ── Condition | Batch controlled (partial) ──")
print(f"    PERMANOVA  p = {p_partial_cc:.6f}  {sig_stars(p_partial_cc)}")
print(
    f"    Omega²       = {fmt(omega2_partial_cc, 4)}  ({_omega_label(omega2_partial_cc)})"
)
    ...
    [
        {
            ...
            "PERMDISP_batch_p": (
                round(p_disp_batch_cc, 6) if not pd.isna(p_disp_batch_cc) else np.nan
            ),
...
def confidence_ellipse(
    ax, x, y, color, n_std=2.448, fill_alpha=0.10, line_alpha=0.75, lw=1.5
):
    if len(x) < 3:
        return
    cov = np.cov(x, y)
...
def draw_pcoa_batch(ax, pcoa_result, meta_df, pc1, pc2, title):
    ...
    for ds in sorted(meta_df["dataset_name"].unique()):
        ...
        confidence_ellipse(
            ax, sub["PC1"].values, sub["PC2"].values, color=DATASET_COLOR[ds]
        )
...
def draw_pcoa_condition(
    ...
    handles = [
        mpatches.Patch(color=COLOR_DICT.get(g, "#999999"), label=g) for g in group_order
    ]
    ax.legend(
        handles=handles,
        loc="lower right",
...
draw_pcoa_condition(
    ...
    omega2=omega2_cc,
    p_batch=p_batch_cc,
    p_disp_batch=p_disp_batch_cc,
    o2_batch=omega2_batch_cc,
    p_part=p_partial_cc,
    o2_part=omega2_partial_cc,
)
