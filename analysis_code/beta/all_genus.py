...
before TSS normalisation via relative_abundance() and all downstream
analysis.
...
if bad_prop.sum() > 0:
    ...
    genus_df = genus_df[~bad_prop.values].reset_index(drop=True)
    metadata = genus_df[["dataset_name", "study_condition", "sampleID"]].copy()
    genus_proportions = genus_proportions[~bad_prop.values].reset_index(drop=True)
else:
    print(
...
# 8a. PERMANOVA + PERMDISP on dataset_name
print("\n  8a. PERMANOVA + PERMDISP on dataset_name (batch effect)...")
...
# 8b. Partial PERMANOVA — condition | dataset
