...
Species-level columns are collapsed to genus level using extract_genus()
before TSS normalisation and all downstream analysis.
...
# 3. AGGREGATION TO GENUS LEVEL
print("Collapsing species to genus level...")

genus_labels   = {col: extract_genus(col) for col in species_df.columns}
original_index = species_df.index

species_df = (
    species_df
    .rename(columns=genus_labels)
    .T
    .groupby(level=0)
    .sum()
    .T
)
species_df.index = original_index

n_genus        = species_df.shape[1]
n_unclassified = (species_df.columns == 'Unclassified_Genus').sum()
print(f"  Collapsed to {n_genus} genus-level features "
      f"({n_genus - n_unclassified} named genera + "
      f"{n_unclassified} Unclassified_Genus column)")
...
if bad_prop.sum() > 0:
    ...
    print(f"  Normalisation passed. "
          f"Matrix: {species_rel.shape[0]} samples × {species_rel.shape[1]} genera.")
...
# 6. BUILD SUBSET — control vs CRC only
...
# 8. CONDITION PERMANOVA + PERMDISP
...
# 9a. PERMANOVA + PERMDISP on dataset_name
print("\n  9a. PERMANOVA + PERMDISP on dataset_name (batch effect)...")
...
# 9b. Partial PERMANOVA
print("\n  9b. Partial PERMANOVA — condition | dataset (residual matrix)...")
...
def vp(omega2_batch, omega2_cond_marginal, omega2_cond_partial):
    ...
    return dict(batch=batch, condition=cond, shared=shared,
                residual=residual, total_explained=batch + cond + shared)
...
print("\n" + "="*75)
print("  FULL STATISTICAL RESULTS  |  All datasets combined  |  Genus level")
...
fig.suptitle(
    'Beta Diversity — Genus Level  |  All Datasets Combined\n'
