...
df = pd.read_csv('./data/filtered_nine_crc_final.tsv', sep='\t', header=None, index_col=0)
df_t = df.T
df_t.columns = df_t.columns.astype(str).str.strip()

# 2. Filter conditions and Clean Age
...
df_age['dataset_name'] = df_age['dataset_name'].map(DATASET_LABEL_MAP).fillna(df_age['dataset_name'])

# Color Palette: Consistent with Alpha Diversity (Control=Red, Adenoma=Blue, CRC=Grey)
color_dict = {
    'control': '#E41A1C',
    'adenoma': '#377EB8',
    'CRC':     '#4D4D4D'
...
plt.title('Age Distribution Across Global CRC Datasets', fontsize=16, weight='bold', pad=25)
...
age_summary = df_age.groupby(['dataset_name', 'study_condition'])['age'].agg(['mean', 'std', 'count']).round(2)
