...
ALL DATASETS COMBINED  |  SPECIES LEVEL  |  TRAIN SET (80% per dataset)
--------------------------------------------------------------------
...
RANDOM_SEED      = 42
OUTPUT_FIG       = 'alpha_diversity_combined_shannon_train.png'
OUTPUT_STATS     = 'alpha_diversity_combined_stats_train.csv'
OUTPUT_DESC      = 'alpha_diversity_combined_descriptive_train.csv'
...
def cliffs_delta_ci(x, y, n_boot=BOOTSTRAP_ITERS, seed=RANDOM_SEED):
    ...
    if total_elements <= MEM_GUARD_ELEMENTS:
        x_boots = rng.choice(x, size=(n_boot, len(x)), replace=True)
        y_boots = rng.choice(y, size=(n_boot, len(y)), replace=True)
        more = np.sum(x_boots[:, :, None] > y_boots[:, None, :], axis=(1, 2))
        less = np.sum(x_boots[:, :, None] < y_boots[:, None, :], axis=(1, 2))
...
def boxplot_stats(values):
    ...
    return {
        'n': len(v), 'median': round(median, 4),
        'Q1': round(q1, 4), 'Q3': round(q3, 4), 'IQR': round(iqr, 4),
        'whisker_min': round(whisker_min, 4), 'whisker_max': round(whisker_max, 4),
    }
...
if unmapped.sum() > 0:
    ...
    print(f"  Excluding {unmapped.sum()} sample(s) with unmapped/excluded "
          f"condition(s): {list(raw_vals)}")
    metadata = metadata[~unmapped].copy()

# Write mapped condition back before splitting
...
df_labeled = prepare_target_class(          # ← utility: adds 'crc_label' column
...
print(f"\n  Pooled train: {len(train_df)} samples  "
      f"(control={(train_df['study_condition']=='control').sum()}, "
      f"CRC={(train_df['study_condition']=='CRC').sum()})")

metadata     = train_df[['dataset_name', 'study_condition', 'sampleID']].copy()
species_cols = [c for c in train_df.columns if c not in METADATA_COLS]
species_df   = (train_df[species_cols]
...
species_norm        = relative_abundance(species_df)        # ← utility function
species_proportions = species_norm[species_cols]


# 4. SHANNON INDEX
print("Calculating Shannon Index H' (bits, log2)...")
metadata = metadata.copy()
metadata['shannon'] = species_proportions.apply(
    lambda x: shannon_entropy(x.values), axis=1).values

nan_shannon = metadata['shannon'].isna()
if nan_shannon.sum() > 0:
    print(f"  Excluding {nan_shannon.sum()} sample(s) with undefined Shannon.")
...
for cond in ['control', 'CRC']:
    ...
    print(f"    {cond:<10}: {n} samples")


# 5. DESCRIPTIVE STATISTICS
print("\n" + "="*75)
print("  DESCRIPTIVE STATISTICS  |  Train set (80% per dataset)")
print("="*75)
...
for cond in ['control', 'CRC']:
    ...
    print(f"  {cond:<12} {n_str:>5} {med_str} {q1_str} {q3_str} "
...
print(f"\nRunning statistical tests + Cliff's delta "
...
# 7. PRINT RESULTS
print("\n" + "="*85)
print("  STATISTICAL RESULTS  |  Train set (80% per dataset)  |  No multiple-testing correction")
print("  Sign convention: d > 0 = control HIGHER diversity | d < 0 = control LOWER")
print("="*85)
for r in results.values():
    print(f"\n  {r['comparison']}")
    print(f"    n_control={r['n_control']}, n_comparison={r['n_comparison']}")
    print(f"    Mann-Whitney p = {r['p_value']:.6f}  {r['significance']}")
    print(f"    Cliff's d     = {r['cliffs_delta']:+.4f}  "
...
for i, cond in enumerate(condition_order):
    ...
    if len(vals) < 2:
        continue
    vparts = ax.violinplot(vals, positions=[i],
...
for i, cond in enumerate(condition_order):
    ...
    ax.hlines(median, i - 0.09, i + 0.09,
              colors='black', linewidth=2.3, zorder=6)
...
for i, cond in enumerate(condition_order):
    ...
    ax.text(i, -0.04, label,
            transform=ax.get_xaxis_transform(),
            ha='center', va='top', fontsize=10.5,
            color='#1A1A1A', linespacing=1.5)

# Axis labels & title
ax.set_ylabel("Shannon Index H' (bits)", fontsize=12,
              fontweight='bold', labelpad=10)
ax.set_xlabel("")
ax.set_title("Alpha Diversity - Species Level\nTrain Set (80% per dataset)",
             fontsize=13, fontweight='bold', pad=14, color='#1A1A1A')

# Legend
legend_elements = [
    Patch(facecolor=COLOR[c], edgecolor=EDGE_COLOR[c],
          alpha=0.80, linewidth=1.2, label=lbl)
    for c, lbl in [('control', 'Control'),
                   ('CRC',     'CRC')]
]
leg = ax.legend(
