...
from pd_utils import (
    aggregate_taxa, relative_abundance, _all_meta,
    prepare_target_class, split_dataset,
)
...
if bad_sum_mask.sum() > 0:
    ...
    print(f"  TSS check passed: {len(row_sums)} samples "
          f"(range: {row_sums.min():.4f}-{row_sums.max():.4f}).")

genus_norm    = relative_abundance(genus_df, feature_cols=feature_cols)  # ← utility function
...
ax.set_xlabel("")
ax.set_title("Alpha Diversity - Genus Level\n",
