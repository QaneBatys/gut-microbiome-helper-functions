...
OUT_DIR = "./results/lgbm_genus_train_val/"
N_JOBS = -1
...
print(f"\n  {'─'*62}")
print(
    f"  {'TOTAL':<30} {len(X_train):>7}  {len(X_test):>6}  "
...
X_train_df = pd.DataFrame(X_train_clr[surviving_taxa].values, columns=surviving_taxa)
X_test_df = pd.DataFrame(X_test_clr[surviving_taxa].values, columns=surviving_taxa)

print(f"\n  After CLR (no scaling for LGBM) :")
...
# 9. HYPERPARAMETER TUNING — train/val (80/20 split)
#    LGBMClassifier n_jobs=1 — grid search handles parallelism.
param_grid = {
    "n_estimators": [500, 1000, 2000],
    "learning_rate": [0.01, 0.05],
    "num_leaves": [7, 15, 31, 63],
    "min_child_samples": [5, 10, 20, 30],
    "feature_fraction": [0.1, 0.2, 0.4, 0.6],
...
print("\n🔍 Tuning hyperparameters (train/val)...")
...
print("=" * 70)
print(
    f"  {'Dataset':<30} {'n':>5}  {'Control':>9}  {'CRC':>6}  "
...
for ds in sorted(X_test["dataset_name"].unique()):
    ...
    print(
        f"  {ds:<30} {len(y_ds):>5}  {n_ctrl:>9}  {n_crc:>6}  "
        f"{auc_ds:>8.4f}  {acc_ds:>10.4f}"
    )

print(f"  {'─'*67}")
print(
    f"  {'OVERALL':<30} {len(y_test):>5}  "
...
fig_roc = plot_auroc(X_test_df, y_test, best_model, label="LightGBM Genus (Train/Val)")
fig_roc.savefig(os.path.join(OUT_DIR, "roc_curve.png"), dpi=300, bbox_inches="tight")

fig_pr = plot_precision_recall(
    X_test_df, y_test, best_model, label="LightGBM Genus (Train/Val)"
)
...
fig_cm = plot_confusion_matrix(
    ...
    y_test,
    best_model,
    title="Confusion Matrix — Test Set (LightGBM Genus)",
